...
from ctd_tools.readers import (
    SbeCnvReader, RbrRskAutoReader, RbrMatlabReader,
    NortekAsciiReader, SbeAsciiReader, RbrAsciiReader,
    AdcpMatlabReader
)
...
class MooringProcessor:
    ...
    def _get_reader_for_file_type(self, file_type: str, file_path: str,
    ...
    def _add_instrument_metadata(self, dataset, instrument_config: Dict[str, Any],
        ...
        dataset['start_time'] = instrument_config.get('start_time', dataset.attrs['deployment_time'])
        dataset['end_time'] = instrument_config.get('end_time', dataset.attrs['recovery_time'])

        return dataset

    def _find_file_tag(self, filename: str, tags: Tuple[str, ...] = ("_000", "_001", "_002")) -> str:
    ...
    def _generate_output_filename(self, mooring_name: str, instrument_config: Dict[str, Any],
    ...
    def _process_instrument(self, instrument_config: Dict[str, Any],
                           yaml_data: Dict[str, Any], input_dir: Path,
                           output_path: Path, mooring_name: str) -> bool:
        ...
        if 'filename' not in instrument_config:
            ...
            self._log_print(f"FILENAME MISSING: Skipping {instrument_name}:{serial}. "
        ...
        output_file = self._generate_output_filename(mooring_name, instrument_config, output_inst_dir)
        ...
        try:
            return self._read_and_write_file(
                ...
                instrument_config, yaml_data, input_dir
            )
    ...
    def _read_and_write_file(self, input_file: Path, output_file: Path,
                            file_type: str, instrument_config: Dict[str, Any],
                            yaml_data: Dict[str, Any], input_dir: Path) -> bool:
        ...
        try:
            reader = self._get_reader_for_file_type(file_type, str(input_file), header_file)
    ...
    def process_mooring(self, mooring_name: str, output_path: Optional[str] = None) -> bool:
        """
        ...
        for instrument_config in yaml_data.get('instruments', []):
            success = self._process_instrument(
                instrument_config, yaml_data, input_dir,
                output_path, mooring_name
            )
            if success:
                success_count += 1

        self._log_print(f"Completed processing: {success_count}/{total_count} instruments successful")
        return success_count > 0


def stage1_mooring(mooring_name: str, basedir: str, output_path: Optional[str] = None) -> bool:
    """
...
if __name__ == "__main__":
    # Your mooring list
    moorlist = ['dsA_1_2018', 'dsB_1_2018', 'dsC_1_2018',
                'dsD_1_2018', 'dsE_1_2018', 'dsF_1_2018']
