...
from oceanarray.stage2 import Stage2Processor, stage2_mooring, process_multiple_moorings_stage2
...
class TestStage2Processor:
    ...
    def test_apply_clock_offset_zero(self, processor, sample_raw_dataset):
        ...
        np.testing.assert_array_equal(sample_raw_dataset.time.values, original_time.values)
    ...
    def test_trim_to_deployment_window_recover_only(self, processor, sample_raw_dataset):
    ...
    def test_trim_to_deployment_window_empty_result(self, processor, sample_raw_dataset):
...
class TestRealDataProcessing:
    ...
    def test_data_setup(self, tmp_path):
        ...
        if not raw_data_file.exists() or not yaml_config_file.exists():
            pytest.skip("Real test data files not found. Run generate_test_data.py first.")
    ...
    def test_process_with_modified_times(self, test_data_setup):
        ...
        with xr.open_dataset(use_file) as ds:
            ...
            with xr.open_dataset(setup['raw_file']) as raw_ds:
                assert len(ds.time) < len(raw_ds.time), f"Expected trimmed data, got {len(ds.time)} vs {len(raw_ds.time)}"
